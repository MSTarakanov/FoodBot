...
def test_poll_selection_stores_key_independently_of_display_text(
    ...
    assert row["option_key"] == PollOptionKey.LUNCH_EAT_IN_OFFICE.value
...
def test_replacing_poll_vote_returns_only_newly_selected_keys(
    database: Database,
) -> None:
    polls = PollRepository(database)
    polls.save(
    ...
    second = polls.replace_selected_options(
        "poll-1",
        42,
        {PollOptionKey.LUNCH_BRING_OWN, PollOptionKey.LUNCH_WOULD_ORDER},
        datetime(2026, 7, 7, 10, 2, tzinfo=UTC),
    )

    assert first == {PollOptionKey.LUNCH_BRING_OWN}
    assert second == {PollOptionKey.LUNCH_WOULD_ORDER}
